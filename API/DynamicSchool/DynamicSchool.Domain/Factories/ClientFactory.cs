        ...
        public Client Buider(IClient client)
        {
            switch (client.ClientTypeId)
            {
                case ClientTypeEnum.Owner :
                    return new ClienteOwner(client.Name, client.Document, client.Cellphone, client.Birthday).SetEmail(client.Email);

                case ClientTypeEnum.Partener :
                    return new ClientPartener(client.Name, client.Document, client.Cellphone, client.Birthday).SetEmail(client.Email);
                case ClientTypeEnum.Simple :
                    return new ClientSimple(client.ClientOrigin.Value, client.Name, client.Document, client.Cellphone, client.Birthday).SetEmail(client.Email);
                default:
